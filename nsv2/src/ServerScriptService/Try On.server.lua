...
function customOnMouseClick(Player, TheirTool)
	...
	local shirt,pants
	if IsBagEquipped(Player) then
		if MouseTarget and (MouseTarget:FindFirstChildOfClass('ClickDetector') or MouseTarget.Parent:FindFirstChildOfClass('ClickDetector')) then
			local ClickDetector = MouseTarget:FindFirstChildOfClass('ClickDetector') or MouseTarget.Parent:FindFirstChildOfClass('ClickDetector')
			shirt,pants = ClickDetector.Parent.Shirt,ClickDetector.Parent.Pants
			if Util:FindAbsoluteAncestor(Folder, ClickDetector) then
				if (TheirTool.Handle.Position - MouseTarget.Position).Magnitude <= ClickDetector.MaxActivationDistance then
					OnClicked(
						Player,
						GetId(shirt),
						GetId(pants),
	...
	if serverConfig.Keys[Player.UserId] == ClientKey then
		...
		elseif Starter == 'TakeOff' then
			...
			if serverConfig.originalClothes[Player.Name] then
				cShirt.ShirtTemplate = serverConfig.templatePrefix:format(serverConfig.originalClothes[Player.Name]['Shirt'])
				cPants.PantsTemplate = serverConfig.templatePrefix:format(serverConfig.originalClothes[Player.Name]['Pants'])
...
function Rewrite(Player, TheirTool)
	...
	table.insert(Connections[Player.UserId], TheirTool.Equipped:Connect(function()
		serverConfig.bagsEquipped[Player.Name] = true
	end))
	table.insert(Connections[Player.UserId], TheirTool.Unequipped:Connect(function()
		serverConfig.bagsEquipped[Player.Name] = nil
	end))
	table.insert(Connections[Player.UserId], TheirTool.Activated:Connect(function()
...
Players.PlayerAdded:Connect(function(Player)
	...
	Util:WaitForChildOfClass(Character,'Shirt',1):andThen(function(result)
		if not result then
			Util:Create("Shirt", {ShirtTemplate = serverConfig.templatePrefix:format('0'), Parent = Character})
		end
	end):catch(error):await()
	Util:WaitForChildOfClass(Character,'Pants',1):andThen(function(result)
		if not result then
