...
if not Gui then repeat task.wait() until Gui ~= nil end
...
local clientConfig = {
	...
	buyIndividualObjectYScale = .395,
	tryOnObjectYScale = .14,
...
function Time.Get()
	...
	table.move(
		clientConfig.loadingTimes,
		1,
		1,
		#clientConfig.archivedLoads,
		clientConfig.archivedLoads
	)
...
local function manageButtons(outfitVisible, tryOnVisible)
	...
	if not exception then
		...
		if (BuyPant.Position.Y.Scale ~= clientConfig.buyIndividualObjectYScale) or (BuyShirt.Position.Y.Scale ~= clientConfig.buyIndividualObjectYScale) then
			manageIndividuals(true)
			BuyPant.Visible = true
			BuyShirt.Visible = true
		end
...
function createViewport(templates)
	return Promise.new(function(resolve)
		...
		for a,b in next,clone:GetChildren() do
			if b.Name == "HumanoidRootPart" then hrp = b end
			if b.Name == "Head" then b['default face']:Destroy() end
			if b:IsA('BasePart') then b.CFrame *= CFrame.Angles(0,math.rad(180),0) end
			b.Parent = newModel
		end
...
function advConnectionUnit()
	for a,b in next,clientConfig._connections.advancedView do
		b:Disconnect()
		b = nil
	end
	return Promise.new(function(resolve, reject)
		clientConfig._connections.advancedView.speedInputTPCS = SpeedInput:GetPropertyChangedSignal('Text'):Connect(function()
			local function sub()
...
function mainConnectionUnit(shirtObject, pantObject)
	for a,b in next,clientConfig._connections.terminal do
		b:Disconnect()
		b = nil
	end
	return Promise.new(function(resolve, reject)
		...
		clientConfig._connections.terminal.buyS = BuyShirt.MouseButton1Click:Connect(function()
			if isOwned(BuyShirt) then return end
			Market:PromptPurchase(Player,shirtObject)
			local temp
			temp = Market.PromptPurchaseFinished:Connect(function(player,assetId,wasPurchased)
		...
		clientConfig._connections.terminal.buyP = BuyPant.MouseButton1Click:Connect(function()
			if isOwned(BuyPant) then return end
		...
		clientConfig._connections.terminal.manType = ManType.MouseButton1Click:Connect(function()
			...
			if ManType.Text == "2D" then
				...
				temp = RunService.Heartbeat:Connect(function(t)
					...
					vpChar:SetPrimaryPartCFrame(vpChar:GetPrimaryPartCFrame() * CFrame.fromEulerAnglesXYZ(0,tonumber(atr),0))
				end)
...
clientConfig._connections.clientEvent = Event.OnClientEvent:Connect(function(Key, ...)
	local Data = {...}
	if Key == 'Open' then
		clientConfig._promise.mainLoad = Promise.new(function(_, reject, onCancel)
			...
			productInfo(shirt, Enum.InfoType.Asset):andThen(function(result)
				infoShirt = result
			end):catch(error):await()

			productInfo(pant, Enum.InfoType.Asset):andThen(function(result)
				infoPant = result
			end):catch(error):await()
			...
			checkAsset(true, shirt):andThen(function(Type)
				if Type == 1 then
					BuyShirt.Text = clientConfig.ownedFormat
			...
			checkAsset(true, pant):andThen(function(Type)
				if Type == 1 then
					BuyPant.Text = clientConfig.ownedFormat
